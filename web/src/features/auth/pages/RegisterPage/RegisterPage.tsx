...
export default function RegisterPage() {
  ...
  const handleCodeInput = useCallback((index: number, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (newDigits.every(d => d !== '')) {
      ...
      submitRegister(newDigits)
    }
  }, [codeDigits, email, firstName, lastName])
  ...
  const handleCodePaste = useCallback((e: React.ClipboardEvent) => {
    ...
    if (newDigits.every(d => d !== '')) submitRegister(newDigits)
  }, [codeDigits, email, firstName, lastName])

  const submitRegister = async (digits: string[]) => {
    ...
    const success = await auth.register({ first_name: firstName, last_name: lastName, email, code: codeValue })
    setIsSubmitting(false)
    if (success) {
      toast('success', '注册成功')
      setTimeout(() => navigate('/library'), 500)
    } else {
      setErrors(prev => ({ ...prev, code: auth.error || '注册失败' }))
      setCodeDigits(['', '', '', '', '', ''])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setErrors({})
    if (step === 1) {
      if (!firstName) { setErrors({ firstName: '请输入姓' }); return }
      if (!lastName) { setErrors({ lastName: '请输入名' }); return }
      if (!email) { setErrors({ email: '请输入邮箱地址' }); return }
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { setErrors({ email: '请输入有效的邮箱地址' }); return }
      setIsLoading(true)
      const ok = await auth.sendCode({ email, type: 'register' })
      setIsLoading(false)
      if (ok) {
        setStep(2)
        setCountdown(60)
  ...
  const handleOAuthCallback = async (provider: OAuthProvider, code: string, state: string) => {
    ...
    if (success) { toast('success', '注册成功'); setShowOAuthModal(false); setTimeout(() => navigate('/library'), 500) }
    else { toast('error', auth.error || 'OAuth 注册失败'); oauthProcessingRef.current = false }
    ...
    <div className="auth-page">
      ...
      <div className="auth-brand">
        <div className="brand-content">
          ...
          <h1 className="brand-title">加入我们</h1>
          <p className="brand-description">开始您的数字化之旅，让知识井井有条</p>
      ...
      <div className="auth-form-container">
        <div className="auth-form-wrapper">
          ...
          <div className="form-header">
            ...
            <p className="form-subtitle">只需几秒钟，即可开始使用</p>
          </div>
          ...
          {activeTab === 'oauth' && (
            <div className="oauth-content">
              <p className="oauth-description">使用您的第三方账号快速注册</p>
          ...
          <div className="form-footer">
            <p className="footer-text">
              已有账号？
              <Link to="/login" className="footer-link">直接登录</Link>
            </p>
          </div>
