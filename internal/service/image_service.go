// Package service provides image management business logic.
package service
...
// ImageService handles image management operations.
...
// ImageRepository defines the data access interface for images.
type ImageRepository interface {
	Create(ctx context.Context, image *model.Image) error
	GetByID(ctx context.Context, id uint) (*model.Image, error)
	ListByOwner(ctx context.Context, ownerID uint, limit, offset int) ([]*model.Image, int64, error)
...
func NewImageService(s storage.Storage, p *processor.Processor, repo ImageRepository, w ImageWorker) *ImageService {
	return &ImageService{
		storage:   s,
		processor: p,
		repo:      repo,
...
func (s *ImageService) Upload(ctx context.Context, req *UploadRequest) (*UploadResponse, error) {
	...
	if err := s.worker.Enqueue(image.ID); err != nil {
		...
		// User can retry processing later via API
		image.Status = model.ImageStatusFailed
		_ = s.repo.Update(ctx, image)
...
func (s *ImageService) GetByID(ctx context.Context, id uint, ownerID uint) (*model.Image, error) {
...
func (s *ImageService) GetByAccessToken(ctx context.Context, token string) (*model.Image, error) {
	return s.repo.GetByAccessToken(ctx, token)
}

// List retrieves images for an owner with pagination.
func (s *ImageService) List(ctx context.Context, ownerID uint, page, pageSize int) ([]*model.Image, int64, error) {
	offset := (page - 1) * pageSize
	return s.repo.ListByOwner(ctx, ownerID, pageSize, offset)
}

// Delete removes an image and its files.
func (s *ImageService) Delete(ctx context.Context, id uint, ownerID uint) error {
	image, err := s.repo.GetByID(ctx, id)
	if err != nil {
		return err
	}
...
func (s *ImageService) ProcessImage(ctx context.Context, imageID uint) error {
...
func (s *ImageService) DownloadFile(ctx context.Context, image *model.Image, variant string) (storage.ReadCloser, string, error) {
	var filePath string
...
func (s *ImageService) CreateGroup(ctx context.Context, ownerID uint, groupType model.ImageGroupType, groupName string, refID *uint, refType *string) (*model.ImageGroup, error) {
...
func (s *ImageService) AddImagesToGroup(ctx context.Context, groupID uint, imageIDs []uint) error {
	// Implementation needed
	return nil
}

// getStorageBasePath returns the base path for local storage.
func (s *ImageService) getStorageBasePath() string {
	if local, ok := s.storage.(*storage.LocalStorage); ok {
		return local.GetFullPath("")
	}
	return ""
}
